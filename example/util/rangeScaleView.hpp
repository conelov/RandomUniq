...
template<typename T>
struct PairKeyDouble {
  using type = std::pair<double, typename std::remove_cvref_t<decltype(std::declval<T>().second)>>;
};
}// namespace aux
...
auto rangeScaleView(auto fromMin, auto fromMax, std::size_t count, auto toMin, auto toMax) {
  ...
  return ranges::views::transform(
           [count, scaler = RangeScaler(fromMin, fromMax, toMin, toMax * count)](auto pair) {
             return typename aux::PairKeyDouble<decltype(pair)>::type{
    ...
    | ranges::views::transform([](auto&& range) {
        return typename aux::PairKeyDouble<decltype(ranges::front(range))>::type{
